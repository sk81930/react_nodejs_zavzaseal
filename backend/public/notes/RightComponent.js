...
const RightComponent = ({ crewMembers, taskData, status, setStatus, priority, setPriority, selectedCrewMembers, setSelectedCrewMembers, crewOptions, setCrewOptions, dropdownOpen, setDropdownOpen,startDate, setStartDate, endDate, setEndDate, handleSubmit  }) => {
      ...
      <MDBRow className="crewDiv">
        ...
        <div className="d-flex align-items-center crewDivInner">
          {selectedCrewMembers.map((member, index) => (
            <img
              key={index}
              src={process.env.REACT_APP_BACKEND+member.profile_image || "https://via.placeholder.com/40"}
              alt={`member${index + 1}`}
